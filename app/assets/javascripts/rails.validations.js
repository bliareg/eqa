...
(function() {
  ...
  $.fn.isValid = function(validators) {
    ...
    if (obj.is('form')) {
      ...
      return validateElement(obj, validatorsFor(this[0].name || this[0].getAttribute('name'), validators));
    }
  };
  ...
  validateForm = function(form, validators) {
    ...
    $.each(validators, function(validator_name, _values) {
      select_element = $(form).find('input[name="' + validator_name + '"]').parents('.dd-container:visible')
      if (select_element && !check_select_validation(select_element)) {
        valid = false;
      }
    });
    if (valid) {
  ...
  ClientSideValidations = {
    ...
    enablers: {
      form: function(form) {
        ...
        ref = {
          'submit.ClientSideValidations': function(eventData) {
            ...
            inputs = $form.find(ClientSideValidations.selectors.inputs);
            if (!$form.isValid(validators)) {
              if (validators['user[password_confirmation]'] && validators['user[password_confirmation]'].confirmation) {
                settings = $form[0].ClientSideValidations.settings.html_settings;
                if (inputs[2] !== undefined) {                                 
                  if(validators['user[password_confirmation]'].confirmation !== undefined) {
                    addErrorPasswordValidation(eventData, form);
                  }    
                }
                else {
                  if (inputs[0].value == inputs[1].value) {
                    element = inputs.slice(1, 2)
      ...
      input: function(input) {
        ...
        $input = $(input);
        form = input.form || $(input).parents(ClientSideValidations.selectors.forms)[0];
        $form = $(form);
        ref = {
          'onfocusout.ClientSideValidations': function() {
        ...
        $input.filter('.json-validate').on('change_json_value', function() {
          $(this).data('changed', true);
          $(this).isValid(form.ClientSideValidations.settings.validators);
        })
      }
    ...
    selectors: {
      inputs: ':input:not(button):not([type="submit"])[name]:visible:enabled, .json-validate',
      validate_inputs: ':input:enabled:visible[data-validate], .json-validate',
    ...
    validators: {
      ...
      local: {
        ...
        json_validate: function(element, options) {
          var input_val = JSON.parse(element.find(':input:checked').val());
          if (!input_val[0] || (!input_val[0].test_plan_id && !input_val[0].test_case_id)) {
            return options.message;
          }
        },
      ...
      remote: {
        uniqueness: function(element, options) {
          ...
          if (options.scope) {
            $.each(options.scope, function( key, value ) {
              var selector = 'input[name = "' + model + '[' + key + ']"]'
              scope[key] = $(selector).val();
            })
          }
          $.ajax({
            type: 'POST',
  ...
  $(document).on(initializeOnEvent, function() {
    result = $(ClientSideValidations.selectors.forms).validate();
    $(document).trigger('validation_load');
    return result;
  });
...
$(document).on('ajax:error', 'form', function(event, request) {
  ajaxErrorValidation(request, $(this));
});
...
function ajaxErrorValidation(request, form) {
  ...
  if (request.status == 422 && form[0].ClientSideValidations && request.responseJSON) {
    ...
    for (var key in request.responseJSON.errors){
      if (formName) {
        input = form.find('#' + formName + '_' + key);
      } else {
        input = form.find('#' + key);
      }
      if (input) {
        form[0].ClientSideValidations.addError(input, request.responseJSON.errors[key]);
      }
    }
  }
}

function addErrorPasswordValidation(eventData, form) {
  element = inputs.slice(2, 3);
  $(form).resetClientSideValidations();
  ClientSideValidations.callbacks.element.fail(element, validators['user[password_confirmation]'].confirmation[0].message, function() {
    return  form.ClientSideValidations.addError(element, validators['user[password_confirmation]'].confirmation[0].message);
  }, eventData);  
}

window.ClientSideValidations.validators.local['unspaces'] = function(element, options) {
  if (element.val() != null)
    if (/^\s+$/i.test(element.val()))
      return options.message;
}